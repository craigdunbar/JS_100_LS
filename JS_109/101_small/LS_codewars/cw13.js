...
sum of the integers to the left of N is equal to the sum of 
the integers to the right of N. 
...
# Output: The lowest index N where the side to the left of N is 
equal to the side to the right of N. 
...
the 3rd position of the array, the sum of left side of the index 
...
# ----------------------------ALGO---------------------------
# need to iterate over the array and slice the array starting at index 0

letft side = arr.slice(0,index)
right side = arr.slice(index + 1)
continue loop until index = arr.length
use reduce to get the sum of the integers in the sliced array
if left === right break and return the index number
return -1 is not equal
*/
function findEvenIndex(arr) {
  for (let count = 0; count < arr.length; count++) {
    let left = arr.slice(0, count).reduce((acc, current) => acc + current,0);
    let right = arr.slice(count + 1).reduce((acc, current) => acc + current,0);
    if (left === right) {
      return count;
    } 
  }
