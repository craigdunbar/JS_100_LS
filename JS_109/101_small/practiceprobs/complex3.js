...
# ----------------------------ALGO---------------------------
8
if integer is greater than arr.legnth return ''
if arr is empty return ''
if integer <= 0 return ''

define a varibale called longest and set to an empty str
define a variable called counter and set to 0

iterate over the string and generate substrings of length k
if the substring length is greater than the count 
set the count to the new length and the variable longest 
will be assigned to the substring

return the longest substring

*/
function longestConsec(arr, num) {
  if (num === 0 || arr.length < num || num <= 0) return ''

  let longest = '';
  let count = 0;

  for (let i = 0; i < arr.length ; i++) {
    let substring = arr.slice(i, i + num).join('')
