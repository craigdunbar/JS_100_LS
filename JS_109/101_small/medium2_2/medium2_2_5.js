/*
# -----------------------INSTRUCTIONS------------------------
Next Featured Number Higher than a Given Value

A featured number (something unique to this exercise) is an odd number 
that is a multiple of 7, with all of its digits occurring exactly once each. 
For example, 49 is a featured number, but 98 is not (it is not odd), 
97 is not (it is not a multiple of 7), and 133 is not (the digit 3 appears twice).

Write a function that takes an integer as an argument and returns 
the next featured number greater than the integer. 
Issue an error message if there is no next featured number.

# --------------------------PROBLEM--------------------------

# Input: integer
# Output: integer representing next featured number
# ---------------------------RULES---------------------------
# Explicit: NOTE: The largest possible featured number is 9876543201.

# Implicit: 

BEYOND?

# --------------------------EXAMPLES-------------------------
featured(12);           // 21
featured(20);           // 21
featured(21);           // 35
featured(997);          // 1029
featured(1029);         // 1043
featured(999999);       // 1023547
featured(999999987);    // 1023456987
featured(9876543186);   // 9876543201
featured(9876543200);   // 9876543201
featured(9876543201);   // "There is no possible number that fulfills those requirements."
# ----------------------------ALGO---------------------------
# a featured number must be - 
an odd number 
divisible by 7
with all digits only occuring once
given the integer argument 
initalise a loop that increments the given number by one
determine if the n ew number is odd, and divisibe by 7 and has unique digits
if it does return the number
else continuw to next iteration

*/
function featured(num) {
  do {
  num += 1;
  let arr = num.toString().split('');
  let unique = [...new Set(arr)];
  let newArr = arr.map((el, idx) => {
    if (el === unique[idx]) {
      return true;
    }
  });
    if ((newArr.every(el => el === true) && num % 2 === 1 && num % 7 === 0)) {
    return num;
    } else {
      continue;
    }
  } while (num <= 9876543201 )
  return "No number can fulfill this.";
}
console.log(featured(12));           // 21
console.log(featured(20));           // 21
console.log(featured(21));           // 35
console.log(featured(997));          // 1029
console.log(featured(1029));         // 1043
console.log(featured(999999));       // 1023547
console.log(featured(999999987));    // 1023456987
console.log(featured(9876543186));   // 9876543201
console.log(featured(9876543200));   // 9876543201
console.log(featured(9876543201));   // "There is no possible number that fulfills those requirements."