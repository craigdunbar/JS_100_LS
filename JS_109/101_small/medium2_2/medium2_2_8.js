...
// The longest sentence has 6 words.
# ----------------------------ALGO---------------------------
# given the string argument make substrings using the endsWith method to
check for . ? or !
then split the substrings into an array of words delimited by spaces and get 
the length of each array.
return the longest of the arrays joined back into a sentence
*/
function longestSentence(str) {
  ...
  for (let idx = 0; idx < arr.length; idx++) {
    ...
    if (currentWord.endsWith('.') || currentWord.endsWith('?') || currentWord.endsWith('!')) {
      let currentSentence = arr.slice(start, idx + 1);
      if (currentSentence.length > longest.length) {
  ...
  console.log(`The longest word has ${longest.length} words`)
}
