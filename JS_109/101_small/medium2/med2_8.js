...
# Input: string
# Output: longest sentence in the string and the number of words 
in that sentence
# ---------------------------RULES---------------------------
# Explicit: 
...
// The longest sentence has 6 words.
# ----------------------------ALGO---------------------------
# iterate over the string and check for sentence ending characters (?!.)
when the character is found push this sentence + end character to an array
...
function longestSentence(str) {
  ...
  for (let idx = 0; idx < str.length; idx++) {
    if (ENDINGS.includes(str[idx])) {
      let currentSentence = (str.slice(start, idx + 1).trim());
      start = idx + 1
      if (currentSentence.split(' ').length > longest.split(' ').length) {
