...
use string.slice() method to spererate the string into an array of sentences
using the delimeiters . ? !
iterate over the array and get the length of each element
if the current element is longer than longest reassign longest to the current string
return the longest string and the sentence showing the number of words
*/
function longestSentence(str) {
  ...
  for (let count = 0; count < str.length; count++) {
    if (str[count].endsWith('.') || str[count].endsWith('!') || str[count].endsWith('?')) {
      let currentSentence = str.slice(start, count + 1).trim();
      start = count + 1;
      if (currentSentence.split(' ').length  > length) {
        length = currentSentence.split(' ').length;
        longest = currentSentence;
      }
    }
}
  console.log(longest);
